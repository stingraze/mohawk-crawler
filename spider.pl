...
open( CONFIG, $config_file )
  or die "Unable to open configuration file: $config_file.";
eval( join( '', <CONFIG> ) );
close(CONFIG);
...
my $server_thread = threads->create( \&create_server, \@files );
...
while ( $iteration <= $max_iterations ) {
    ...
    foreach my $url (@url_queue) {
        ...
        if ( should_process_url($url) ) {
            ...
            if ( $ignore_robot_rules || is_robot_allowed($url) ) {
                ...
                if ($page_text) {
                    ...
                    if ( $guess_encoding_problems
                        && has_encoding_problems($page_text) )
                    {
                        print
                          "----> Page has unsupported character encoding.\n";
                        next;
                    }
      ...
      # Added on 04/05/16: get html snippet when meta description does not exist
                    if ( $description eq '' ) {
                        $description = get_page_summary( $page_text,
                    ...
                    if ( $description || $keywords || $title ) {
                        ...
                        print $socket get_db_record( $url, $keywords,
                            $description, $title );
                    }
                    ...
                    if ( $iteration < $max_iterations ) {
                        ...
                        my @imagemaps;
                        (@anchors) =
                          $page_text =~ m/<a[^>]*href\s*=\s*["']([^"'>]*)"/gsxi;
                        (@frames) = $page_text =~
                          m/<i?frame[^>]*src\s*=\s*["']([^"'>]*)"/gsxi;
                        (@imagemaps) = $page_text =~
                          m/<area[^>]*href\s*=\s*["']([^"'>]*)"/gsxi;
                        my @all_links = ();
                        ...
                        foreach my $anchor (@anchors) {
                            my $new_url =
                              get_fully_quialified_url( $url, $anchor );
                        ...
                        print
"----> New pages found               : $added_pages\n";
                    }
                }
                print "----> Remaining pages for iteration : "
                  . ( $#url_queue + 1 - $processed_this_iteration ) . "\n\n";
...
sub should_process_url {
    ...
    if ($domain_max_timeout) {
        ...
        my ( $server_host, $port, $document ) =
          $rest =~ m|^//([^:/]*):*([0-9]*)/*([^:]*)$|;
        $domain_times{$iteration}{$server_host} ||= time();
        return 0
          if (
            time() - $domain_times{$iteration}{$server_host} >
            $domain_max_timeout );
    }
...
sub is_robot_allowed {
    ...
    unless ( defined $robots{$key} ) {
        load_robot_rules( $server_host, $port );
    }
...
sub get_page {
    ...
    if ( $response->is_success ) {
        unless ( check_content_type( $response->header('content-type') ) ) {
            print "Excluded because of content type ("
...
sub get_page_summary {
    ...
    $content =~
s/<(font|br|center|!DOCTYPE|html|head|meta|base|title|basefont|body|style|abbr|address|b|bdi|bdo|big|blockquote|defines|cite|code|del|dfn|em|font|i|ins|kbd).*?>//gsxi;
    $content =~
s/<(mark|meter|pre|progress|q|rp|ruby|s|rt|samp|small|strike|strong|sub|sup|time|tt|u|var|wbr|div|span|main|section|article|aside|details|summary|dir).*?>//gsxi;
    $content =~
s/<(a|link|nav|frame|frameset|noframes|iframe|acronym|abbr|address|b|bdi|bdo|big|code|del|dfn|font|i|ins|mark|meter|progress|rp|rt|ruby|small|strong|sub).*?>//gsxi;
    $content =~
s/<(sup|time|tt|u|var|wbr|br|hr|p|textarea|a|ul|ol|li||dl|dt|dd|article|aside|summary|h1|h2|h3|h4|h5|h6).*?>//gsxi;

    # Remove all irrelevant html end tags
    $content =~
s/<\/(font|br|center|!DOCTYPE|html|head|meta|base|title|basefont|body|style|abbr|address|b|bdi|bdo|big|blockquote|defines|cite|code|del|dfn|em|font|i|ins)>//gsxi;
    $content =~
s/<\/(kbd|mark|meter|pre|progress|q|rp|ruby|s|rt|samp|small|strike|strong|sub|sup|time|tt|u|var|wbr|div|span|main|section|article|aside|details|summary|dl)>//gsxi;
    $content =~
s/<\/(dt|dd|a|link|nav|frame|frameset|noframes|iframe|acronym|abbr|address|b|bdi|bdo|big|code|del|dfn|font|i|ins|mark|rp|rt|ruby|small|strong|sub|sup|time)>//gsxi;
    $content =~
s/<\/(meter|progress|tt|u|var|wbr|br|hr|p|textarea|a|ul|ol|li|article|aside|summary|h1|h2|h3|h4|h5|h6)>//gsxi;
...
sub create_server {
    ...
    unless ($@) {
        ...
        async( \&saver, $q, $files )->detach;

        while (( not defined($number_of_clients) )
            or ( $number_of_clients != 0 ) )
        {
            ...
            if ($socket) {
                $number_of_clients++;
                async( \&{handle_connection}, $socket, $q )->detach;

            }

            #say "Number of clients: " . $number_of_clients;
        }
...
sub saver {
    ...
    open( DB, ">", $output_file_name )
      or die "Unable to open $output_file_name.";
