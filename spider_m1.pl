...
#my $id_number_form = 0;
#$id_number_form = $ARGV[0];
#print "the id:" . $id_number_form; 
#Get epoch and make it part of file name/
$epoc = time();
#Debug
#print $id_number_form;
use Getopt::Std;
use vars qw($opt_i $opt_r);

use Encode::Guess qw/ euc-jp shiftjis utf8 7bit-jis euc-kr euc-cn big5-eten /;
...
#my $config_file = "/var/www/cgi-bin/spider.conf";
		...
		if(should_process_url($url)){
			...
			if ($ignore_robot_rules || is_robot_allowed($url)){
				...
				if ($page_text){
					...
					#1-7-2020 Changed this part (commented out for test)
					...
					if ($iteration < $max_iterations) {
			      	...
			      	my $new_url = get_fully_qualified_url($url, $anchor);
